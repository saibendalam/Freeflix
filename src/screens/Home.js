...
// This listener handles the case where the app is woken up from the Universal or Deep Linking
   Linking.addEventListener('url', this.appWokeUp);
    fetch('https://api.freeflix.in/public/live-streams')
      ...
      fetch('https://api.freeflix.in/public/languages')
      .then((response) => {

        return response.json()
      })
      .then((responseJSON) => {
         responseJSON.result.unshift({'_id':'all','name':'all'})
        this.setState({
          LangList: responseJSON.result,
      ...
      fetch('https://api.freeflix.in/public/categories')
      .then((response) => {
    ...
    return (
      ...
      <View style={styles.container}>
        ...
        <Modal isVisible={this.state.isLangModalVisible}
          style={{ justifyContent: "flex-end",paddingBottom:'9.3%',
          margin: 0,width:wid}}
          onBackdropPress={() => {
            this.setState({ 
              isLangModalVisible: false, 
...
>
            ...
            <View style={{ justifyContent:'space-between', height: '55%', width: '100%', alignItems: 'center', backgroundColor: mainclr }}>
            <FlatList
          data={this.state.LangList}
          renderItem={({ item }) => (
            <View style={{justifyContent:'space-between',alignItems:'center',paddingTop:10, flexDirection: 'column', margin: 1,width:'50%',height:'100%' }}>
          ...
          onPress={() => {
            if (this.state.language.indexOf(item._id) !== -1) {
              ...
              this.setState({
                ...
                <Image source={require('./images/'+item.name[0])} style={{ height: 28, width: 28, tintColor:'yellow' }} />
              <Text  style={{ color: this.state.language.indexOf(item._id) !== -1 ? yellow : 'white',textAlignVertical: "center"}}>
        ...
        />
              ...
              <Icon name="chevron-down" onPress={() => {  this.setState({ 
              isLangModalVisible: false, 
              refresh: !this.state.refresh});
...
<View style={{ justifyContent:'space-between', height: '55%', width: '100%', alignItems: 'center', backgroundColor: mainclr }}>
          ...
          activeOpacity = { .5 }
          onPress={() => {
            if (this.state.categories.indexOf(item._id) !== -1) {
              ...
              <Text  style={{ color: this.state.categories.indexOf(item._id) !== -1 ? yellow : 'white',textAlignVertical: "center"}}>
        {item.name[0].toUpperCase() +  
                  item.name.slice(1)}
                                            </Text>
                </TouchableOpacity>
